...
function! GetOrgFoldtext()
	if mode() == 'i'
		...
		if has_key(b:org_folding_cache, v:foldstart) &&
					\ len(b:org_folding_cache[v:foldstart]) == 2
			return b:org_folding_cache[v:foldstart][1]
		endif
python << EOF
...
EOF
	...
	if exists('b:foldtext')
		...
		unlet b:foldtext
		if mode() == 'i' && has_key(b:org_folding_cache, v:foldstart)
			if len(b:org_folding_cache[v:foldstart]) == 1
				call add(b:org_folding_cache[v:foldstart], l:tmp)
			else
				let b:org_folding_cache[v:foldstart][2] = l:tmp
			endif
		endif
		return l:tmp
	endif
