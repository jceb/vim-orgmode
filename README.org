An attempt to port [[http://orgmode.org][org-mode]] to vim.
...
** Building a Debian Package
   ...
   directory of this plugin to build the debian package:

   dpkg-buildpackage -us -uc

   For installing the plugin form the resulting vim-orgmode_X.X.X-X.deb
   file, please refer to the Installation section.
...
** Navigator plugin
*** add mapping to repeat the last movement, e.g. <C-l> and <C-h>
...
*** implement tests for ]]
*** DONE implement mappings for visual and operator mode in a better way
...
*** DONE indentation with C-t,C-d doesn't work when not on a heading
...
*** implement support for umlauts in tags
...
** Documentation
*** document current functionality based on the [[http://orgmode.org/guide/index.html][Org-mode compact guide]]
...
*** DONE bug in implementation of accessing the buffers causes weird behavior when changing the buffer
*** DONE reimplement syntax file (the current version is from hsitz)
