...
** 0.6.0                  					 :unreleased:
*** Added
    - Keybinding to create plainlist item directlyi(=<localleader>c[l/L]= for
      plainlist item [below/above]). (closes issue #190)
...
*** Removed
    - Using =<localleader>c[n/N]= to create new plainlist item following
      current plainlist item. Now these keybindings will unconditionally create
      empty checkbox.
*** Fixed
